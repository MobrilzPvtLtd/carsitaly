...
@section('content')
    <x-backend.layouts.show :data="$tour">
        ...
        <x-backend.section-header>
            ...
            <x-slot name="toolbar">
                ...
                <a class="btn btn-primary m-1" data-toggle="tooltip" href="{{ route('backend.tour.index') }}" title="List"><i class="fas fa-list"></i> List</a>
            ...
            <div class="col">
                <div class="table-responsive">
                    <table class="table-bordered table-hover table">
                        <tr>
                            ...
                            <td>
                                <img class="user-profile-image img-fluid img-thumbnail"
                                    src="{{ asset('public/uploads/tour/') . '/' . $tour->image}}"
                                    style="width: 100px;" />
                            </td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td>{{ $tour->created_at }} @yield('name')<small>
                                ({{ $tour->created_at->diffForHumans() }})</small>
                            </td>
                        </tr>

                        <tr>
                            <th>{{ __('labels.backend.users.fields.updated_at') }}</th>
                            <td>{{ $tour->updated_at }}<br />
                                <small>({{ $tour->updated_at->diffForHumans() }})</small>
                            </td>
                        </tr>
                    </table>
                </div>
                ...
                <div class="py-4 text-end">
                    <a class="btn btn-danger mt-1" data-method="DELETE" data-token="{{ csrf_token() }}"
                        ...
                        href="{{ route('backend.tour.destroy', $tour) }}" title="{{ __('labels.backend.delete') }}"><i class="fas fa-trash-alt"></i> Delete</a>
                </div>
            </div>
        </div>
